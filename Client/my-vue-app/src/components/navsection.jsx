        ...
        <span className="text-black-500 w-auto" >NORDSTROM</span>
         ...
         <div className=" sm:flex  w-[300em] ml-10 sm:ml-10 lg:ml-40">
          ...
          <div className="flex  space-x-2 text-[1em] bg-white">
            <NavItem label="Sign&nbsp;In&nbsp;>" subItems={[<button style={{color:"white",width:"10em" ,borderRadius:"0px"}} >create account</button>, 'Pants', 'Shoes']} />
            <NavItem label="Sign&nbsp;In&nbsp;>" subItems={['Shirts', 'Pants', 'Shoes']} />
            <NavItem label="Sign&nbsp;In&nbsp;>" subItems={['Shirts', 'Pants', 'Shoes']} />
...
const NavItem = ({ label, subItems }) => {
  return (
    ...
    {subItems && (
      <div className="hidden group-hover:block absolute left-0 top-full bg-white-800 p-4 rounded shadow-md px-[2.5em] pb-[4em]" style={{backgroundColor:"white"}}>
        
        {subItems.map((item) => (
          <div key={item} className="text-xs sm:text-sm md:text-base lg:text-lg">
            <br />
            {item}
          </div>
        ))}
